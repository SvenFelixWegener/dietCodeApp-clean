    ...
    var body: some View {
                ...
                LazyVStack(spacing: 16) {
                    ...
                    ForEach(mealTypes, id: \.self) { meal in
                        MealDashboardCard(
                            ...
                            onDeleteEntry: { vm.deleteEntry($0.id, meal: meal) },
                            onDuplicateEntry: { vm.duplicateEntry($0.id, meal: meal) },
                            onEditEntry: { editingEntry = EntryEditContext(meal: meal, entry: $0) }
                        )
                    }
                }
                .padding(16)
        ...
        .overlay {
            if vm.isLoading || vm.isSaving || vm.isAnalyzingIngredients {
                ProgressView(vm.isAnalyzingIngredients ? "Zutaten werden analysiert …" : "Lädt …")
                    .padding()
...
struct MealDashboardCard: View {
    ...
    private var groupedEntries: [(key: String, value: [MealEntry])] {
        ...
        VStack(alignment: .leading, spacing: 10) {
            ...
            if isExpanded {
                if entries.isEmpty {
                    ...
                    ForEach(groupedEntries, id: \.key) { _, items in
                        if let first = items.first,
                           first.groupType == "ai_ingredients",
                           items.count > 1 {
                            AIIngredientGroupCard(
                                entries: items,
                                onDeleteGroup: { items.forEach(onDeleteEntry) },
    ...
    var body: some View {
        NavigationStack {
            FoodSearchView(
                vm: vm,
                token: token,
                onBarcodeTap: { isScannerShown = true },
            ...
            .onChange(of: selectedPhoto) { _, newValue in
                ...
                Task {
                    if let data = try? await newValue.loadTransferable(type: Data.self) {
                        await vm.analyzeIngredientsImage(
                            container: container,
                            token: token,
                            imageData: data
                        )
        ...
        .sheet(isPresented: $showAnalysisReview) {
            if let analysis = vm.ingredientAnalysis {
        ...
        .sheet(isPresented: $isScannerShown) {
            BarcodeScannerView(
                onCodeScanned: { code in
                    ...
                    Task {
                        await vm.applyScannedBarcode(code, container: container, token: token)
                    }
                },
...
struct PortionEditorSheet: View {
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 16) {
            ...
            HStack {
                ForEach([50.0, 100, 150], id: \.self) { preset in
                    Button("\(Int(preset))g") {
                        grams = preset
                    }
            ...
            List {
                ...
                Section("Erkannte Zutaten") {
                    ForEach(analysis.ingredients) { ingredient in
                        Toggle(
                            isOn: Binding(
                                get: { selections[ingredient.id.uuidString, default: true] },
                                set: { selections[ingredient.id.uuidString] = $0 }
                            )
                        ) {
            ...
            .toolbar {
                ToolbarItem(placement: .topBarTrailing) {
                    Button("Als Gruppe hinzufügen") {
                        let selected = analysis.ingredients.filter {
